...
class LegoEnvMNIST(gym.Env):
    def __init__(self,
        ...
        self.observation_space = gym.spaces.Dict({'adjacency' : gym.spaces.Box(low = 0, high = 1, shape = (self.max_node_num, self.max_node_num), dtype = np.float32),
                                                  ...
                                                  'available_actions' : gym.spaces.Box(low = 0., high = 1., shape = (self.max_node_num, self.action_space.n), dtype = np.int32),
    ...
    def reset(self):
        ...
        available_actions_matrix = np.zeros((self.max_node_num, len(LIST_RULES_2_4) * 2))
        available_actions_matrix[:1] = self.get_available_actions_2(init_node_coordinates)
    ...
    def step(self, actions):
        pivot_fragment_index, relative_action = actions
    ...
    def render(self):
        utils_meshes.get_mesh_bricks(self.bricks_)
...
#        updated_edge_attributes = np.zeros(shape = (92, *A.shape), dtype=np.float32)
        ...
        available_actions_matrix = np.zeros((self.max_node_num, len(LIST_RULES_2_4) * 2))
        available_actions_matrix[:X.shape[0]] = updated_available_actions
...
#            return reward_change * self.target_num_brick
    ...
    def get_available_actions_2(self, pivot_brick_coordinate):
        if self.bricks_ is not None:
            possible_actions = np.array([self.check_action_availability(pivot_brick_coordinate, idx) for idx in range(len(LIST_RULES_2_4) * 2)])

            return possible_actions.astype(np.int32)[None, :]

    def check_action_availability(self, pivot_brick_coordinate, action):
