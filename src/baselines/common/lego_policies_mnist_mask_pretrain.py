...
class PolicyWithValue_Lego_Mnist(tf.Module):
    ...
    def __init__(self, ac_space, policy_network, target_network, value_network=None, estimate_q=False):
        """
        Parameters:
        ----------
...
class PolicyWithValue_Lego_Mnist_MultiGNN(tf.Module):
    ...
    def __init__(self, ac_space, policy_network, target_network, hidden_dim, target_hidden_dim, value_network=None, estimate_q=False):
        ...
        # self.init_node_fc = fc_relu(4 + target_hidden_dim, 'init_fc', hidden_dim, init_scale=np.sqrt(2))
        self.init_node_fc = Init_FC(hidden_dim=hidden_dim, target_dim=target_hidden_dim, layer_type=0)

        # self.init_edge_fc = fc_relu(4 + target_hidden_dim, 'init_fc', hidden_dim, init_scale=np.sqrt(2))
        self.init_edge_fc = Init_FC(hidden_dim=hidden_dim, target_dim=target_hidden_dim, layer_type=1)
    ...
    def step(self, observation, pivot_mask, offset_mask, training=True):
        ...
        # masked_for_pivot_node_logits = for_pivot_node_logits + negative_node_mask
...
class PolicyWithValue_Lego_Mnist_NoGNN(tf.Module):
    """
    Encapsulates fields and methods for RL policy and value function estimation with shared parameters
    """
    ...
    def __init__(self, ac_space, policy_network, target_network, hidden_dim, target_hidden_dim, value_network=None, estimate_q=False):
        ...
        # self.init_node_fc = fc_relu(4 + target_hidden_dim, 'init_fc', hidden_dim, init_scale=np.sqrt(2))
        self.init_node_fc = Init_FC(hidden_dim=reshaped_hidden_dim, target_dim=target_hidden_dim, layer_type=0)

        self.gcn_init = Lego_FC(policy_network, node_dim=reshaped_hidden_dim, hidden_dim=reshaped_hidden_dim, target_info_dim=target_hidden_dim, edge_dim=reshaped_hidden_dim, layer_num=1)

        self.gcn_block = []

        for i in range(2):
            temp_gcn = Lego_FC(node_dim=reshaped_hidden_dim, hidden_dim=reshaped_hidden_dim, target_info_dim=target_hidden_dim, edge_dim=reshaped_hidden_dim, layer_num=i+2)

            self.gcn_block.append(temp_gcn)

        self.pivot_gcn_init = Lego_FC(policy_network=None, node_dim=reshaped_hidden_dim, hidden_dim=reshaped_hidden_dim, target_info_dim=target_hidden_dim, edge_dim=reshaped_hidden_dim, layer_num=1)
...
class PolicyWithValue_Lego_Mnist_NoEdgeGNN(tf.Module):
    """
    Encapsulates fields and methods for RL policy and value function estimation with shared parameters
    """
    ...
    def __init__(self, ac_space, policy_network, target_network, hidden_dim, target_hidden_dim, value_network=None, estimate_q=False):
        ...
        self.gcn_init = Lego_Message_Passing_No_Edge(policy_network, node_dim=reshaped_hidden_dim, hidden_dim=reshaped_hidden_dim, target_info_dim=target_hidden_dim, edge_dim=reshaped_hidden_dim, layer_num=1)

        self.gcn_block = []

        for i in range(2):
            temp_gcn = Lego_Message_Passing_No_Edge(node_dim=reshaped_hidden_dim, hidden_dim=reshaped_hidden_dim, target_info_dim=target_hidden_dim, edge_dim=reshaped_hidden_dim, layer_num=i+2)

            self.gcn_block.append(temp_gcn)

        self.pivot_gcn_init = Lego_Message_Passing_No_Edge(policy_network=None, node_dim=reshaped_hidden_dim, hidden_dim=reshaped_hidden_dim, target_info_dim=target_hidden_dim, edge_dim=reshaped_hidden_dim, layer_num=1)
