...
from baselines.a2c.utils import fc, fc_relu, conv, ortho_init
...
class PolicyWithValue_Lego_Artificial(tf.Module):
    ...
    def __init__(self, ac_space, policy_network, target_network, value_network=None, estimate_q=False):
        ...
        self.init_node_fc = Init_FC(layer_type=0)

        self.init_edge_fc = Init_FC(layer_type=1)

        self.gcn_init = Lego_Message_Passing_Artificial(policy_network, node_dim=hidden_dim, hidden_dim=hidden_dim, target_info_dim=target_hidden_dim, edge_dim=hidden_dim, layer_num=1)

        self.gcn_block = []

        for i in range(2):
            temp_gcn = Lego_Message_Passing_Artificial(node_dim=hidden_dim, hidden_dim=hidden_dim, target_info_dim=target_hidden_dim, edge_dim=hidden_dim, layer_num=i+2)
...
class PolicyWithValue_Lego_Artificial_MultiGNN(tf.Module):
    ...
    def __init__(self, ac_space, policy_network, target_network, hidden_dim, target_hidden_dim, value_network=None, estimate_q=False):
        ...
        self.init_node_fc = Init_FC(layer_type=0, hidden_dim=hidden_dim, target_dim=reshaped_target_hidden_dim)

        self.init_edge_fc = Init_FC(layer_type=1, hidden_dim=hidden_dim, target_dim=reshaped_target_hidden_dim)

        self.gcn_init = Lego_Message_Passing_Artificial_MultiGNN(policy_network, node_dim=hidden_dim, hidden_dim=hidden_dim, target_info_dim=reshaped_target_hidden_dim, edge_dim=hidden_dim, layer_num=1)

        self.gcn_block = []

        for i in range(2):
            temp_gcn = Lego_Message_Passing_Artificial_MultiGNN(node_dim=hidden_dim, hidden_dim=hidden_dim, target_info_dim=reshaped_target_hidden_dim, edge_dim=hidden_dim, layer_num=i+2)

            self.gcn_block.append(temp_gcn)

        self.pivot_gcn_init = Lego_Message_Passing_Artificial_MultiGNN(policy_network=None, node_dim=hidden_dim, hidden_dim=hidden_dim, target_info_dim=reshaped_target_hidden_dim, edge_dim=hidden_dim, layer_num=1)
        ...
        self.pivot_classifier = fc(hidden_dim + reshaped_target_hidden_dim, 'piv', 1, init_scale=np.sqrt(2))

        # Based on the action space, will select what probability distribution type
        self.pdtype = make_pdtype((None, hidden_dim + reshaped_target_hidden_dim), ac_space, init_scale=np.sqrt(2))

        if estimate_q:
            assert isinstance(ac_space, gym.spaces.Discrete)
            self.value_fc = fc(hidden_dim, 'q', ac_space.n, init_scale=np.sqrt(2))
        else:
            self.value_fc = fc(hidden_dim + hidden_dim + reshaped_target_hidden_dim, 'vf', 1, init_scale=np.sqrt(2))
